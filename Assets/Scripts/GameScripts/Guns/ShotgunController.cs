...
public class ShotgunController : MonoBehaviour
{
    ...
    void Start()
    {
        // Поиск объекта с компонентом PelletSpawn и получение его transform
        var pelletSpawn = FindObjectOfType<PelletSpawn>();
        if (pelletSpawn != null)
            barrelEnd = pelletSpawn.transform;
        else
            Debug.LogError("Объект с компонентом PelletSpawn не найден!");
    ...
    private void FireShotgun()
    {
        for (int i = 0; i < pelletsPerShot; i++)
        {
            ...
            // Визуализация траектории (видно только в редакторе)
            Debug.DrawRay(barrelEnd.position, shootDirection * 10f, Color.yellow, 30f);
        }
    }
}
